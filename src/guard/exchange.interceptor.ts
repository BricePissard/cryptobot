...
import { Injectable, ExecutionContext, CallHandler, NestInterceptor, HttpException, HttpStatus } from '@nestjs/common';
...
export class ExchangeInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    try {
      ...
      if (!allowed) {
        throw new HttpException('Assets not in allowed list', HttpStatus.NOT_ACCEPTABLE);
      }
      return next.handle();
    } catch (error) {
      throw error;

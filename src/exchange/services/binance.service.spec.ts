...
describe('ExchangeService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [BinanceExchange, BotConfigService, { provide: SecretsService, useValue: secrets }]
    }).compile();
    ...
    const LONG_TIME = 1621164085102;
    jest.spyOn(service, 'getTime').mockResolvedValue(LONG_TIME);
  });
  ...
  it('should reject when price data is incorrect', (done) => {
    ...
    price.subscribe({
      complete: () => done(),
      error: (error) => {
        expect(error).toMatchObject(new HttpException('Failed to get price info', HttpStatus.BAD_REQUEST));
        done();
      }
    });
  });
      ...
      next: (x) => {
        expect(x.balances).toMatchObject(
          allAsset.balances.filter(
            (item) => config.rebalanceProfiles.find((a) => a.asset === item.currency_code) !== undefined
          )
        );
      },
      complete: () => done()
  ...
  it('should correctly calculate output balance', async (done) => {
    ...
    of(result).subscribe({
      next: (x) => {
        const FINAL_BALANCE = 1.02256442;
        expect(x.total.amount).toBeCloseTo(FINAL_BALANCE);
  ...
  it('should find the amount of asset to sell when not specified', async () => {
    ...
    const FINAL_BALANCE = 1;

    expect(result).toEqual(orderResponse.data);
    expect(getBalance).toBeCalledWith('USDT');
    expect(getBalance).toBeCalledTimes(FINAL_BALANCE);
  });

  it('should not find the amount of asset to sell when specified', async () => {
    const getBalance = jest.spyOn(service, 'getBalance').mockResolvedValueOnce('' as any);

    jest.spyOn(httpClient, 'post').mockReturnValueOnce(of(orderResponse));
    const FINAL_AMOUNT = 0.01;
    const result = await service.sell('BTC', 'USDT', FINAL_AMOUNT);
  ...
  it('should throw when asset to sell is not found', async () => {
    ...
    await expect(result).rejects.toThrow(new HttpException('Could not find an asset to sell.', HttpStatus.BAD_REQUEST));
    const FINAL_BALANCE = 1;
    expect(getBalance).toBeCalledWith('USDT');
    expect(getBalance).toBeCalledTimes(FINAL_BALANCE);
  });
  ...
  it('should not find the amount of asset to buy when specified', async () => {
    ...
    jest.spyOn(httpClient, 'post').mockReturnValueOnce(of(orderResponse));
    const AMOUNT_TO_BUY = 0.01;
    const result = await service.buy('DOGE', 'BTC', OrderType.Market, AMOUNT_TO_BUY);
  ...
  it('should throw when asset to buy with is not found', async () => {
    ...
    await expect(result).rejects.toThrow(
      new HttpException('Could not calculate total available asset.', HttpStatus.INTERNAL_SERVER_ERROR)
    );
    const FINAL_BALANCE = 1;
  ...
  it('should rebalance correctly when amount is not specified', async () => {
    ...
    const buyRequest = jest.spyOn(httpClient, 'post').mockReturnValueOnce(of(orderResponse));
    const result = await service.buy('ETH', 'BTC');
  ...
  it('should use lot size to calculate correct purchase / sell amount', async () => {
    ...
    const FINAL_BALANCE = 6.2;

    expect(purchaseAmount - remainder).toBe(FINAL_BALANCE);
  });
});

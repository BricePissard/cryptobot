...
export class BinanceExchange extends ExchangeService {
  baseURL: string =
    process.env.NODE_ENV === 'production' ? 'https://api.binance.com' : 'https://testnet.binance.vision';
  key = {
    ...
    const response = this.httpService
      ...
      .pipe(
        ...
        catchError((err) => {
          console.error(err.response.data);
          return throwError(
            new HttpException(`${JSON.stringify(err.response.data)}`, HttpStatus.INTERNAL_SERVER_ERROR)
          );
        })
      )
      .toPromise();

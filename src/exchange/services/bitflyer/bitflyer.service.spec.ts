...
describe('ExchangeService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [BitFlyerExchange, BotConfigService, { provide: SecretsService, useValue: secrets }]
    }).compile()
  ...
  it('should find the amount of asset to sell when not specified', async () => {
    const getBalance = jest.spyOn(service, 'getBalance').mockResolvedValue(allAsset)
  ...
  it('should not find the amount of asset to sell when specified', async () => {
    const getBalance = jest.spyOn(service, 'getBalance').mockResolvedValue('' as any)
  ...
  it('should buy the dip', async () => {
    ...
    expect(buyLimit.mock.calls.map((item) => item[3])).toEqual([0.03858, 0.0868, 0.1984])
    expect(buyLimit.mock.calls.map((item) => item[4])).toEqual([50000 * 0.9, 50000 * 0.8, 50000 * 0.7])

...
export class ExchangeController {
  ...
  async makeSellOrder(@Body() botReq: BotRequest): Promise<any> {
    ...
    return this.service.sell(botReq.asset.trim(), botReq.denominator.trim(), botReq.amount)
  }
  ...
  async makeBuyOrder(@Body() botReq: BotRequest): Promise<any> {
    ...
    assertIsString(botReq.asset)
    assertIsString(botReq.denominator)
    return this.service.buy(botReq.asset.trim(), botReq.denominator.trim(), OrderType.Market, botReq.amount)
  }
  ...
  async makeBuyOrderWithFiat(@Body() botReq: BotRequest): Promise<any> {
    ...
    assertIsString(botReq.denominator)
    try {
      const assetPrice = await this.service.getPrice(botReq.asset, botReq.denominator).toPromise()
      ...
      throw new HttpException('Could not fetch price feed data', HttpStatus.INTERNAL_SERVER_ERROR)
    }
  }
  ...
  async makeBuyDipOrders(@Body() botReq: BotRequest): Promise<any> {
    ...
    return this.service.bidDips(botReq.asset.trim(), botReq.denominator.trim(), botReq.dip)
  }
